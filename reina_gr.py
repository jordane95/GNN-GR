...
def reina_apply(raw_datasets, key, value, num_proc):
    ...
    index_data_list[key] = [entry[key] for entry in train_set] # List[str]

    index_data_list[value] = [entry[value] for entry in train_set] # List[int]
    ...
    for set_name, query_data in query_data_dict.items():
        ...
        qbar.close()
        datasets_new[set_name] = [(entry['id'], refs) for entry, refs in zip(query_data, lines_reina)]
    return datasets_new
...
def reina_offline(data_path, key, value, num_proc):
    ...
    for set_name in ['dev', 'test', 'train']:
        ...
        with open(os.path.join(data_path, 'reina_' + set_name + '.tsv'), 'w') as fpw:
            dataset = datasets_new[set_name]
            for qid, neg_ids in dataset:
                fpw.write(f"{qid}\t{','.join(map(str, neg_ids))}\n")
...
def load_g2s_dataset(data_path):
    ...
    g2s_datasets['dev'] = list(map(linearize_graph, dev_set))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--key_column', type=str, default='kg',
                        help='REINA key')
    parser.add_argument('--value_column', type=str, default='id',
                        help='REINA value')
